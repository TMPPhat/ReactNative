...
import apiProduct from '../../api/apiProduct'; // Import API Product để lấy ảnh
...
interface ExtendedOrderItem extends OrderItemData {
...
export default function OrdersScreen() {
  ...
  const fetchOrders = async () => {
    if (!user) return;
    try {
      // 1. Lấy danh sách tất cả sản phẩm để map ảnh (Tránh gọi API detail quá nhiều lần)
      const productsRes = await apiProduct.getAllProducts();
      const productMap = new Map();
      if (productsRes.results) {
        productsRes.results.forEach((p: any) => {
            const imgUrl = p.image && p.image.length > 0 ? p.image[0].url : null;
            if (imgUrl) {
                productMap.set(p.id, imgUrl);
            }
        });
      }
  ...
  // --- Handle Re-Order (Đặt lại) ---
  const handleReOrder = (order: OrderDisplay) => {
    Alert.alert(
      "Đặt lại đơn hàng",
      "Giỏ hàng hiện tại sẽ bị xóa. Bạn có chắc muốn đặt lại các món trong đơn này?",
      [
        ...
        { 
          ...
          onPress: () => {
            ...
            order.items.forEach(item => {
                const productId = item.product && item.product.length > 0 ? item.product[0].id : 0;
                if (productId) {
                    addToCart({
                        id: productId,
        ...
        { 
          ...
          onPress: async () => {
            try {
                ...
                await apiOrder.updateOrderStatus(orderId, 'cancelled');

                await apiOrderStatus.create({
                    order_id: [orderId],
                    status: 'cancelled',
      ...
      {isLoading ? (
        ...
        >
            ...
            filteredOrders.map((order) => {
                ...
                const isPending = order.status?.value === 'pending';
                ...
                <View key={order.id} style={styles.orderCard}>
                    ...
                    {order.items.length > 0 ? (
                        // SỬA: Chỉ hiển thị 3 sản phẩm đầu tiên
                        <>
                            ...
                            {/* Hiển thị thêm nếu có hơn 3 sản phẩm */}
...
const styles = StyleSheet.create({
  ...
  itemImage: { width: 50, height: 50, borderRadius: 8, backgroundColor: '#f3f4f6', marginRight: 12 }, // Style ảnh
  ...
  // Style cho text "và X món khác"
  moreItemsText: { fontSize: 12, color: '#6b7280', fontStyle: 'italic', marginTop: 4, textAlign: 'center' },
